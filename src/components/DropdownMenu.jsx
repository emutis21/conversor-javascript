  ...
  useEffect(() => {
    if (listRef.current) {
      const clampedIndex = Math.max(
        0,
        Math.min(selectedIndex, filteredCurrencies.length - 1)
      );
        ...
        >
          ...
          {filteredCurrencies.length === 0 && searchTerm !== "" && (
            <motion.li
              ...
              style={{ opacity: 0 }}
              className={
                " w-32  border-b border-gray-500 px-5 py-3 transition-all duration-100 cursor-not-allowed"
              }
            >
